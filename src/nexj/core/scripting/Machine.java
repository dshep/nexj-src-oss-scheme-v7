...
public class Machine
{
   ...
   /**
    * Creates and pushes an exception handler.
    * @param handler The exception handling code.
    * @param finalizer The finalizer code.
    */
   public void pushExceptionHandler(PCodeFunction handler, PCodeFunction finalizer)
   {
      ...
      m_exceptionHandlerStack[m_nExceptionTop++] = Boolean.FALSE;
   }
    ...
    */
   public void setTry(PCodeFunction fun, PCodeFunction handler, PCodeFunction finalizer)
   {
   ...
   public void setStack(Object[] stack)
   {
      ...
      int nStackCount = stack.length - nExceptionCount - 1;

      m_nTop = m_nStackBarrier + nStackCount;

      if (m_nTop > m_nStackLength)
      {
         m_nStackLength = m_nTop + 32;
         m_stack = new Object[m_nStackLength];
      }

      m_nExceptionTop = m_nExceptionBarrier + nExceptionCount;

      if (m_nExceptionTop > m_exceptionHandlerStack.length)
      {
         m_exceptionHandlerStack = new Object[m_nExceptionTop + 16];
      }
      ...
      for (;;)
      {
         ...
         catch (Throwable e)
         {
            ...
            if (m_nExceptionTop != m_nExceptionReturn)
            {
               ...
               if (m_exceptionHandlerStack[m_nExceptionTop - 1] != Boolean.FALSE)
               {
                  push(e);
                  m_nArgCount = 1;
   ...
   public class MachineState
   {
      ...
      /**
       * @see this{@link #getNextInstructionLocation(BreakpointSite)}
       *
      ...
      public PCodeLocation findNextInstructionLocation(char instruction, Predictor stepper, boolean bStepIn)
      {
         ...
         switch (instruction)
         {
            ...
            default:
               ...
               if (instruction >= CALL_INTRINSIC)
               {
                  ...
                  else if (ifun == Intrinsic.SYS_FINALIZE)
                  {
                     nextFun = (PCodeFunction)m_exceptionHandlerStack[m_nExceptionTop - 2];
                     nNextPC = 0;
                  }
                  else if (ifun == Intrinsic.CALL_CC || ifun == Intrinsic.CALL_WITH_CURRENT_CONTINUATION)
                  {
                     int nArgCount = nextFun.code[pc + 1];
                     ...
                     { 
                        ...
                        {
                           nNextPC = 0;
                           nextFun = (PCodeFunction)getArg(0, nArgCount);
                        }
                        else if (value instanceof Intrinsic.Continuation)
                        {
                           visitor = new TopStackFrameVisitor();
                           visitContinuation(visitor, (Intrinsic.Continuation)value);
                           nextFun = visitor.getFunction();
                        ...
                        {
                           // See CALL case: any unary intrinsic functions should be supported.
                           nNextPC += getPCodeLength(instruction);
                        }
                     }
                     else
         ...
         if (nextFun != null)
         {
            ...
            if (nNextPC >= nextFun.code.length)
            {
               visitor = new TopStackFrameVisitor();
               visitStackTrace(visitor);
            ...
            return new PCodeLocation(nextFun, nNextPC);
         }
         else
         {
            return null;
      ...
      public PCodeFunction findNextExceptionHandler()
      {
         ...
         else if (m_exceptionHandlerStack[m_nExceptionTop - 1] != Boolean.FALSE)
         {
